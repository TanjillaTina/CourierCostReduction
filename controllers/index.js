...
var indexPage = (req, res) => {
  Request.find({
    shiperOrshippingTo: { $size: 1 },
    conignee: { $size: 1 },
    finaldone: true
  })
    .distinct("receivedyear")
    .then(result => {
      receivedyear = result;

      Request.find({ shiperOrshippingTo: { $size: 1 }, conignee: { $size: 1 } })
        .distinct("userDetail.reqyear")
        .then(results => {
          reqyear = results;
...
var viewChart = (req, res) => {
  ...
  if (!yearr) {
    ...
    Request.find({ finaldone: true, receivedyear: yearr })
      .distinct("comname")
      .then(results => {
        /////////*********************** */
        ...
        Request.aggregate([
          ...
          {
            $group: {
              _id: { comname: "$comname", receivedmonth: "$receivedmonth" },
...
var viewChart2 = (req, res) => {
  ...
  if (!yearr) {
    ...
    Request.find({ "userDetail.reqyear": yearr })
      .distinct("userDetail.usermail")
      .then(results => {
        ...
        //console.log("Printing here Final Array"+ finalArray);
        ...
        Request.aggregate([
          { $match: { "userDetail.reqyear": yearr } }, //filtering staeps
          {
            $group: {
              _id: {
                reqyear: "$userDetail.reqyear",
                reqmonth: "$userDetail.reqmonth",
                usermail: "$userDetail.usermail",
          ...
          .then(result => {
            ...
            console.log("Printing here Final Array" + finalArray);
            ////start of  finding req year and month for the downnload menu/////////////////
            Request.find({
              shiperOrshippingTo: { $size: 1 },
              conignee: { $size: 1 }
            })
              ...
              .then(results => {
                ...
                Request.find({
                  ...
                  .distinct("userDetail.reqmonth")
                  .then(resultsz => {
                    reqmonth = resultsz;

                    /////////////////////////////////// start of finding the cost per user

                    res.render("indexchart2", {
                      year: yearr,
                      datai: finalArray,
                      names: namesara,
                      reqyear: reqyear,
                    ...
                    /////////////////////////////////// end of finding the cost per user
                  })
                  .catch(err => {
                    throw err;
                  });
...
var downExcMonth = (req, res) => {
  ...
  Request.aggregate([
    {
      $match: {
        "userDetail.reqyear": yearr.toString(),
        "userDetail.reqmonth": monthd.toString()
      }
    }, //filtering staeps
    {
      $group: {
    ...
    .then(results => {
      ...
      var datai =
        "\t" +
        " Total Request Details of Month  " +
        monthd +
        "of Year " +
        yearr +
        "\t" +
      ...
      for (k = 0; k < results.length; k++) {
        datai =
          datai +
          results[k]._id.username +
          "\t" +
          results[k]._id.usermail +
          "\t" +
          results[k].totalreqs +
          "\n";
        console.log(datai);
  ...
  Request.aggregate([
    ...
    .then(results => {
      ...
      Request.aggregate([
        ...
        {
          $group: {
            _id: { month: "$receivedmonth", year: "$receivedyear" },
            totalbox: { $sum: 1 }
          }
        }
      ])
        .then(resultss => {
          ...
          console.log(
            "Printing distinct totalnumber of boxes for month and year"
          );
          ...
          for (var i = 0; i < reqs.length; i++) {
            for (var j = 0; j < reqss.length; j++) {
              if (
                reqs[i]._id.month == reqss[j]._id.month &&
                reqs[i]._id.year == reqss[j]._id.year
              ) {
                var persa =
                  (100 * parseInt(reqs[i].totalcount)) /
                  parseInt(reqss[j].totalbox);
                var box = {
                  percen: Math.floor(persa),
...
var costPerPersonChart = (req, res) => {
  var yearr = req.body.selectyear3;
  if (!yearr) {
    req.flash("error_msg", "You must select a year to view that page");
    res.redirect("/");
  } else {
    Request.find({ receivedyear: yearr, finaldone: true })
      .distinct("userDetail.usermail")
      .then(results => {
        //   console.log("Prinitg Maiks " + results);
        ...
        Request.aggregate([
          { $match: { receivedyear: yearr, finaldone: true } }, //filtering staeps
          {
            $group: {
              _id: {
                reqyear: "$receivedyear",
                reqmonth: "$receivedmonth",
                usermail: "$userDetail.usermail",
                username: "$userDetail.username"
              },
              totalAmount: { $sum: "$cost" }
            ...
            Request.find({
              ...
              .then(results => {
                ...
                Request.find({
                  ...
                  .distinct("receivedmonth")
                  .then(resultsz => {
                    reqmonth = resultsz;

                    /////////////////////////////////// start of finding the cost per user

                    res.render("costPerPersonChart", {
                      year: yearr,
                      datai: finalArray,
                      names: namesara,
                      reqyear: reqyear,
...
var dwnCurentMonthCost= (req, res) => {
  var d = new Date();
  var monthd = d.getMonth() + 1;
  var yearr = d.getFullYear();
  var ara = new Array();
  ...
  Request.aggregate([
    {
      $match: {
        "receivedyear": yearr.toString(),
        "receivedmonth": monthd.toString(),
        "finaldone": true
      }
    }, //filtering staeps
    {
    ...
    .then(results => {
      ...
      for (k = 0; k < results.length; k++) {
        datai =
          ...
          results[k]._id.usermail +
          "\t" +
          results[k].totalAmount +
          "\n";
        console.log(datai);
      }
      var finame = "Cost Per Person Sheet of month " + monthd + " of year " + yearr;
      //var finame="RequestSheet of month ";
      var filename = finame + ".xls";
      fs.writeFile(filename, datai, err => {
        if (err) throw err;
...
var dwnAnyMonthCost =  (req, res) => {
 ...
 // console.log("Month " + months + " Year " + years);
  ...
  Request.aggregate([
    { $match: { "receivedyear": years, "receivedmonth": months, "finaldone": true } }, //filtering staeps
    {
      $group: {
        _id: {
          username: "$userDetail.username",
    ...
    .then(results => {
      ...
      var finame = "Cost Per Person Sheet of month " + months + " of year " + years;
      //var finame="RequestSheet of month ";
      var filename = finame + ".xls";
      fs.writeFile(filename, datai, err => {
        if (err) throw err;
